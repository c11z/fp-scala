...
class ListsSuite extends FunSuite {
  ...
  test("max of mixed positive and negative numbers") {
    assert(max(List(-1, -100, 5, 123)) === 123)
  }

  test("max with only negative numbers") {
    assert(max(List(-1, -3, -6, -100)) === -1)
  }

  test("max throws java.util.NoSuchElementException if list is empty") {
